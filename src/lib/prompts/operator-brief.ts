...
function formatMetrics(snapshot: DailySnapshot): string {
  ...
  if (m.activeUsers) lines.push(`- Active users (DAU/WAU/MAU): ${m.activeUsers}`);
...
export function buildBriefPrompt(
  ...
  userParts.push("---");
  userParts.push("");
  userParts.push(`Using all the context above, generate my Daily Operator Brief following the exact output format below. Be specific. Be uncomfortable. Be useful.
...
In 3-5 sentences, state the unvarnished reality of where the business stands today.
...
### 4. Today's Priority Stack
PRIORITY #1 (Non-Negotiable), #2 (Only If #1 Complete), #3 (Nice-to-Have), plus DO NOT DO TODAY list.
...
### 7. Closing Question
One uncomfortable question the founder should sit with today.`);
